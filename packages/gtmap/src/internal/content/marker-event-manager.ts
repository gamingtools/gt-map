...
export class MarkerEventManager {
	...
	emitMarker(name: MarkerEventName, payload: MarkerEventData): void {
		...
		for (const fn of Array.from(set)) {
			try {
				...
				/* expected: user marker event handler may throw */
			}
		}
	}
	...
	setMarkerData(payloads: Record<string, unknown | null | undefined>): void {
		try {
			...
			this.deps.debugWarn('setMarkerData', e);
		}
	}
	...
	handleMarkersChanged(nextIds: Set<string>): void {
		try {
			...
			// Prune stale marker data entries for removed markers
			for (const id of this._markerData.keys()) {
				if (!nextIds.has(id)) this._markerData.delete(id);
			}
		} catch (e) {
			this.deps.debugWarn('marker hover cleanup', e);
		}
	}
}
