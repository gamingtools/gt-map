...
export class IconRenderer {
  ...
  setMarkers(markers: Array<MarkerRenderData | { x: number; y: number; type: string; size?: number; rotation?: number; zIndex?: number; iconScaleFunction?: IconScaleFunction | null }>) {
    ...
    for (const m of markers || []) {
      if ('id' in (m as Record<string, unknown>)) {
        ...
        norm.push({ id: `m${idx++}`, x: mm.x, y: mm.y, type: mm.type, ...(mm.size !== undefined ? { size: mm.size } : {}), ...(mm.rotation !== undefined ? { rotation: mm.rotation } : {}), ...(mm.zIndex !== undefined ? { zIndex: mm.zIndex } : {}) });
      }
    }
    this.markers = norm;
  ...
  setDecals(decals: Array<MarkerRenderData | { x: number; y: number; type: string; size?: number; rotation?: number; zIndex?: number }>) {
    ...
    for (const d of decals || []) {
      if ('id' in (d as Record<string, unknown>)) {
        ...
        norm.push({ id: `d${idx++}`, x: dd.x, y: dd.y, type: dd.type, ...(dd.size !== undefined ? { size: dd.size } : {}), ...(dd.rotation !== undefined ? { rotation: dd.rotation } : {}), ...(dd.zIndex !== undefined ? { zIndex: dd.zIndex } : {}) });
      }
    }
    this.decals = norm;
    this.buffers.rebuildTypeData(this.markers, this.decals, this.atlas);
  ...
  getMarkerInfo(scale = 1): Array<{
    ...
    for (const m of this.markers) {
      ...
      out.push({ id: m.id, index: idx, x: m.x, y: m.y, w, h, type: m.type, anchor: { ax: a.ax * scale, ay: a.ay * scale }, ...(m.rotation !== undefined ? { rotation: m.rotation } : {}), icon: meta });
      idx++;
    }
    return out;
  }
  ...
  private createProgram(gl: WebGLRenderingContext, vsSrc: string, fsSrc: string): WebGLProgram | null {
    ...
    if (!gl.getShaderParameter(vs, gl.COMPILE_STATUS)) {
      try { console.warn('instanced VS error', gl.getShaderInfoLog(vs)); } catch { /* expected */ }
      return null;
    }
    ...
    if (!gl.getShaderParameter(fs, gl.COMPILE_STATUS)) {
      try { console.warn('instanced FS error', gl.getShaderInfoLog(fs)); } catch { /* expected */ }
      return null;
    }
    ...
    if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
      try { console.warn('instanced link error', gl.getProgramInfoLog(prog)); } catch { /* expected */ }
      return null;
    }
    return prog;
  }
