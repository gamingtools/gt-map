...
export default class LeafletMapFacade {
	...
	constructor(container: HTMLElement | string, options?: LeafletMapOptions) {
		...
		const init: any = {};
		if (options?.center) {
			const c = toLngLat(options.center);
			init.center = c;
		}
		...
		if (options?.maxBounds) {
			const sw = Array.isArray(options.maxBounds) ? { lat: options.maxBounds[0][0], lng: options.maxBounds[0][1] } : toLngLat(options.maxBounds.getSouthWest());
			const ne = Array.isArray(options.maxBounds) ? { lat: options.maxBounds[1][0], lng: options.maxBounds[1][1] } : toLngLat(options.maxBounds.getNorthEast());
	...
	getCenter(): [number, number] {
		return toLeafletLatLng(this._map.center.lng, this._map.center.lat);
	}
	getZoom(): number {
		return this._map.zoom;
	}
	getMinZoom(): number {
		return (this._map as any).minZoom;
	}
	getMaxZoom(): number {
		return (this._map as any).maxZoom;
	}
	get pointerAbs(): { x: number; y: number } | null {
		return (this._map as any).pointerAbs ?? null;
	}
	get events(): EventBus {
		return this._map.events as EventBus;
	}

	// Common convenience methods (bridge to impl)
	panTo(latlng: LeafletLatLng, _opts?: any): this {
		const p = toLngLat(latlng);
		this._map.setCenter(p.lng, p.lat);
		return this;
	}
	flyTo(latlng: LeafletLatLng, zoom?: number, _opts?: any): this {
		const p = toLngLat(latlng);
		this._map.setCenter(p.lng, p.lat);
		if (typeof zoom === 'number') this._map.setZoom(zoom);
		return this;
	...
	zoomIn(delta?: number): this {
		...
		this._map.setZoom(this._map.zoom + d);
		return this;
	}
	...
	stop(): this {
		/* no animation queue to stop yet */ return this;
	}
	invalidateSize(_options?: any): this {
	...
	setWheelSpeed(v: number) {
		(this._map as any).setWheelSpeed?.(v);
		return this;
	}
	setWheelCtrlSpeed(v: number) {
		(this._map as any).setWheelCtrlSpeed?.(v);
		return this;
	}
	setGridVisible(on: boolean) {
		(this._map as any).setGridVisible?.(on);
		return this;
	}
	setFpsCap(v: number) {
		(this._map as any).setFpsCap?.(v);
		return this;
	}
	setWrapX(on: boolean) {
		(this._map as any).setWrapX?.(on);
		return this;
	}
	setMaxBounds(bounds?: LeafletBoundsLike | null) {
		if (!bounds) {
			(this._map as any).setMaxBoundsPx?.(null);
			return this;
		}
		const sw = Array.isArray(bounds) ? { lat: bounds[0][0], lng: bounds[0][1] } : toLngLat((bounds as any).getSouthWest());
		const ne = Array.isArray(bounds) ? { lat: bounds[1][0], lng: bounds[1][1] } : toLngLat((bounds as any).getNorthEast());
		(this._map as any).setMaxBoundsPx?.({ minX: sw.lng, minY: sw.lat, maxX: ne.lng, maxY: ne.lat });
		return this;
	}
	setMaxBoundsViscosity(v: number) {
		(this._map as any).setMaxBoundsViscosity?.(v);
		return this;
	}
	...
	__addLayer(layer: { onAdd: (m: any) => void }) {
		this._layers.add(layer);
		try {
			layer.onAdd(this);
		} catch {}
	}
	__removeLayer(layer: { onRemove: (m: any) => void }) {
		if (this._layers.delete(layer)) {
			try {
				layer.onRemove(this);
			} catch {}
		}
