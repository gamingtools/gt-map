...
easeWrap.style.position = 'absolute';
easeWrap.style.left = '8px';
easeWrap.style.top = '210px';
easeWrap.style.background = 'rgba(255,255,255,0.9)';
easeWrap.style.border = '1px solid #bbb';
easeWrap.style.borderRadius = '4px';
easeWrap.style.padding = '6px 8px';
easeWrap.style.font = '12px/1.2 system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, Noto Sans, Helvetica, Arial, sans-serif';
easeWrap.style.zIndex = '11';

const wheelEaseLabel = document.createElement('label');
wheelEaseLabel.textContent = 'Wheel Ease (ms)';
wheelEaseLabel.style.display = 'block';
wheelEaseLabel.style.marginBottom = '4px';

const wheelEaseInput = document.createElement('input');
wheelEaseInput.type = 'range';
wheelEaseInput.min = '80';
wheelEaseInput.max = '400';
wheelEaseInput.step = '10';
wheelEaseInput.value = String(map.easeBaseMs);
wheelEaseInput.style.width = '140px';
const wheelEaseVal = document.createElement('span');
wheelEaseVal.textContent = `${map.easeBaseMs}`;
wheelEaseVal.style.marginLeft = '6px';
const wheelRow = document.createElement('div');
wheelRow.style.display = 'flex';
wheelRow.style.alignItems = 'center';
wheelRow.appendChild(wheelEaseInput);
wheelRow.appendChild(wheelEaseVal);

wheelEaseInput.addEventListener('input', () => {
  const base = parseInt(wheelEaseInput.value, 10) || 150;
  wheelEaseVal.textContent = `${base}`;
  map.setEaseOptions({ easeBaseMs: base });
});

const pinchEaseLabel = document.createElement('label');
pinchEaseLabel.textContent = 'Pinch Ease (ms)';
pinchEaseLabel.style.display = 'block';
pinchEaseLabel.style.margin = '6px 0 4px 0';

const pinchEaseInput = document.createElement('input');
pinchEaseInput.type = 'range';
pinchEaseInput.min = '60';
pinchEaseInput.max = '300';
pinchEaseInput.step = '10';
pinchEaseInput.value = String(map.pinchEaseMs);
pinchEaseInput.style.width = '140px';
const pinchEaseVal = document.createElement('span');
pinchEaseVal.textContent = `${map.pinchEaseMs}`;
pinchEaseVal.style.marginLeft = '6px';
const pinchRow = document.createElement('div');
pinchRow.style.display = 'flex';
pinchRow.style.alignItems = 'center';
pinchRow.appendChild(pinchEaseInput);
pinchRow.appendChild(pinchEaseVal);

pinchEaseInput.addEventListener('input', () => {
  const val = parseInt(pinchEaseInput.value, 10) || 140;
  pinchEaseVal.textContent = `${val}`;
  map.setEaseOptions({ pinchEaseMs: val });
});

const pinchToggleLabel = document.createElement('label');
pinchToggleLabel.style.display = 'inline-flex';
pinchToggleLabel.style.alignItems = 'center';
pinchToggleLabel.style.gap = '6px';
pinchToggleLabel.style.marginTop = '6px';
pinchToggleLabel.textContent = 'Ease Pinch';
const pinchToggle = document.createElement('input');
pinchToggle.type = 'checkbox';
pinchToggle.checked = !!map.easePinch;
pinchToggle.addEventListener('change', () => map.setEaseOptions({ easePinch: pinchToggle.checked }));
pinchToggleLabel.appendChild(pinchToggle);

easeWrap.appendChild(wheelEaseLabel);
easeWrap.appendChild(wheelRow);
easeWrap.appendChild(pinchEaseLabel);
easeWrap.appendChild(pinchRow);
easeWrap.appendChild(pinchToggleLabel);
container.appendChild(easeWrap);
